...
const getAllEmailTemplates = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      status: 'success',
      message: emailTemplates.length === 0 ? 'No email templates found' : 'Email Templates retrieved successfully',
      emailTemplates,
    });
  } catch (error) {
...
const getEmailTemplateById = async (req, res) => {
  ...
  try {
    const emailTemplate = await emailtemplateModel.findById(templateId).populate('companyId');
    if (emailTemplate) {
...
const addEmailTemplate = async (req, res) => {
  ...
  try {
    ...
    if (companyId === undefined) {
      return res.status(404).json({ status: 'error', message: 'User or company not found' });
    }
...
const deleteEmailTemplate = async (req, res) => {
  try {
    ...
    const isEmailTemplateInUse = await templateModel.countDocuments({ emailTemplate: emailTemplate.id });

    if (isEmailTemplateInUse) {
      return res.status(400).json({
        status: 'error',
        message: 'This email template is being used in one or more templates and cannot be deleted.',
      });
    }
